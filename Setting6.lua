...
function SliderModule.Key(config)
	...
	local function closeAdvFrame()
		...
		if KeyBroad then KeyBroad.Visible = false end
		self._waiting = false
		self._preview = nil
		keyReg_exitWaiting(self)
	end
	...
	local function openAdvFrame()
		registerOpen(self)   -- đóng Color/Gradient đang mở + block các inst khác
		self._waiting = true
		self._preview = self._value   -- preview bắt đầu = key hiện tại
		keyReg_enterWaiting(self)

		-- Hiện key hiện tại lên AdvKeyText (kèm màu)
		if AdvKeyText then
	...
	if isAdvance and advanceFrame then
		...
		if KeyBroad then
			...
			-- Đếm số lần mỗi text đã gặp để xử lý duplicate (SHIFT x2, \\ x2)
			local usedCount = {}

			-- Sort theo LayoutOrder để đảm bảo thứ tự đúng khi xử lý duplicate
			local keyFrames = {}
			...
			for _, keyFrame in ipairs(keyFrames) do
				-- Tìm label tên "Key", fallback sang TextLabel đầu tiên trong frame
				...
				-- Tính suffix để xử lý duplicate (SHIFT lần 1 → "SHIFT", lần 2 → "SHIFT_2")
			...
			-- ── TableButton: toggle hiện/ẩn KeyBroad ────────────────
			if TableButton then
				...
				table.insert(self._conns, TableButton.MouseButton1Click:Connect(function()
					KeyBroad.Visible = not KeyBroad.Visible
	...
	function self:destroy()
		closeAdvFrame()
		if decorTween1 then decorTween1:Cancel() end
		if decorTween2 then decorTween2:Cancel() end
		if colorTween1 then colorTween1:Cancel() end
		if colorTween2 then colorTween2:Cancel() end
		for _, conn in ipairs(self._conns) do conn:Disconnect() end
		self._conns = {}
		keyReg_remove(self)
		createdSliders[frame] = nil
