...
--        [Attr] StoreName   = "PlayerData_v1"   ← tên DataStore (bắt buộc)
...
--             Ví dụ: "PlayerData_v1_123456789"
--    "name" → key = StoreName .. "_" .. PlayerName (lower)
--             Ví dụ: "PlayerData_v1_heroname"
--    "seed" → key = StoreName .. "_" .. hash(KeySeed .. UserId)
--             Ví dụ: "PlayerData_v1_a3f9c2" (obfuscated)
--
...
local PLAYER_STORES = {
	...
	-- ── Store 1: Data chính của player ──────────────────────────
	{
		-- Cấu hình store
		config = {
			storeName     = "PlayerData_v1",  -- tên DataStore thật trên Roblox
		...
		schema = {
			...
			meta = {
				coins      = { default=0,   min=0,  max=-1,    oldName=nil,          readOnly=false },
				gems       = { default=0,   min=0,  max=-1,    oldName="diamonds",   readOnly=false },
				level      = { default=1,   min=1,  max=100,   oldName=nil,          readOnly=false },
				exp        = { default=0,   min=0,  max=-1,    oldName="experience", readOnly=false },
				username   = { default="",  min=nil,max=nil,   oldName="playerName", readOnly=true  },
				isPremium  = { default=false,min=nil,max=nil,  oldName=nil,          readOnly=false },
				totalKills = { default=0,   min=0,  max=-1,    oldName="kills",      readOnly=false },
				stats = {
					isTable = true,
					subMeta = {
						kills  = { default=0, min=0, max=-1,  oldName=nil, readOnly=false },
						deaths = { default=0, min=0, max=-1,  oldName=nil, readOnly=false },
						wins   = { default=0, min=0, max=-1,  oldName=nil, readOnly=false },
					},
				},
				inventory = {
					isTable = true,
					subMeta = {
						sword  = { default=false, readOnly=false },
						shield = { default=false, readOnly=false },
						bow    = { default=false, readOnly=false },
					},
				},
			},
		},
...
function API.DMCreate(id, setting)    if DataModule then return DataModule:Create(id, setting) end end
function API.DMGet(id)                if DataModule then return DataModule:Get(id) end end
function API.DMGetOrCreate(id, s)     if DataModule then return DataModule:GetOrCreate(id, s) end end
