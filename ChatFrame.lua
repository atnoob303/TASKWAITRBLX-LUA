...
TextChatService.MessageReceived:Connect(function(message)
	...
	newMessage.Name    = tostring(player.UserId) .. "_" .. tostring(message.MessageId)
	...
	if ContentBR then
		...
		if contentScale > SETTINGS.MAX_SCALE then
			-- Cắt text đến khi scale về TRUNCATE_TARGET_SCALE
			local text = fullText
			...
			-- Sync lại ContentChat nếu bị truncate
			newMessage.ContentChat.Text = ContentBR.Text
			finalContentScale = SETTINGS.MAX_SCALE  -- giữ MAX dù đã cắt
		end
	end
	...
	setupHoverButton(newMessage, fullText, displayName, newMessage.NamePl.Time.Text, player.UserId)
