...
local function _buildOldKeys(config, player)
	...
	local base   = (prefix and prefix ~= "") and prefix
		or (config.oldStoreName and config.oldStoreName ~= "" and config.oldStoreName)
		or config.storeName
...
function PlayerData:RegisterFromFolder(folder)
	...
	local config = {
		...
		oldKeyPrefix  = (attrs.OldKeyPrefix ~= "" and attrs.OldKeyPrefix) or nil,
		oldStoreName  = (attrs.OldStoreName ~= "" and attrs.OldStoreName) or nil,
		oldKeySeed    = (attrs.OldKeySeed   ~= "" and attrs.OldKeySeed)   or nil,
	}
...
function PlayerData:Load(player, storeName)
	...
	-- ════════════════════════════════════════════
	--  FALLBACK KEY CŨ
	--  Kích hoạt khi key mới không có data VÀ
	--  config có oldKeyPrefix / oldStoreName / oldKeyMode
	-- ════════════════════════════════════════════
	local migratedFromOldKey = nil  -- lưu key cũ đã dùng để log

	if (not ok or not raw) and (config.oldKeyPrefix or config.oldStoreName or config.oldKeyMode) then
		...
		if config.oldKeyPrefix or config.oldStoreName then
			...
			for _, c in ipairs(candidates) do
				...
				if tok and traw then
					...
					foundRaw = traw
					print(string.format(
						"PlayerData: [%s] Key cũ tìm thấy (prefix/%s) '%s' → migrate sang '%s'",
						player.Name, c.mode, c.key, key
					))
					break
				end
			end
		...
		if not foundRaw then
			...
			if fuzzyKey then
				...
				if tok and traw then
					...
					foundRaw = traw
					print(string.format(
						"PlayerData: [%s] Key cũ tìm thấy (fuzzy) '%s' → migrate sang '%s'",
						player.Name, fuzzyKey, key
					))
				end
			end
		end
		...
		if foundRaw then
			...
			-- Xóa key cũ nếu ở store khác (tránh double data)
			if config.oldStoreName and config.oldStoreName ~= storeName then
