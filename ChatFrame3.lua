...
local DEFAULT_SETTINGS = {
	...
	CHAT_MODE               = 2,
	BUBBLE_MAX_PLAYERS      = 5,      -- Số người tối đa có bubble cùng lúc
...
-- Lịch sử chat toàn bộ (dùng cho HistoryChat panel)
-- entry: { userId, displayName, text, channelId, time, tag }
-- Giới hạn 75 dòng mới nhất
local globalChatLog   = {}
...
-- ── Thêm tin vào globalChatLog ───────────────────────────────
local function appendChatLog(userId, displayName, text, channelId)
	table.insert(globalChatLog, {
		userId      = userId,
	...
	-- Giữ tối đa HISTORY_MAX dòng
	while #globalChatLog > HISTORY_MAX do
		table.remove(globalChatLog, 1)
	end
end
...
local function openHistoryChat(userId, channelId)
	...
	-- Lọc log theo channelId
	local filtered = {}
	for _, entry in ipairs(globalChatLog) do
		if entry.channelId == channelId then
	...
	local totalHeight = 12  -- top + bottom padding
	local FONT        = Enum.Font.GothamMedium
	local TEXT_SIZE   = 13
	local PANEL_WIDTH = HistoryChat.AbsoluteSize.X > 0 and HistoryChat.AbsoluteSize.X - 16 or 260

	for i, entry in ipairs(filtered) do
		-- Format: [#Tag] DisplayName: text
		local tagStr = ""
		if entry.channelId and entry.channelId ~= "DEFAULT" and entry.channelId ~= "Global" then
			tagStr = "[#" .. entry.channelId:sub(1,8):upper() .. "] "
		end
		local line = tagStr .. entry.displayName .. ": " .. entry.text

		-- Tính chiều cao dòng (TextService)
		local ok, bounds = pcall(function()
			return game:GetService("TextService"):GetTextSize(
				line, TEXT_SIZE, FONT, Vector2.new(PANEL_WIDTH, 9999)
			)
		end)
		local lineH = (ok and bounds.Y or TEXT_SIZE) + 4
		...
		-- Highlight tin nhắn của player đang xem
		if entry.userId == userId then
			label.TextColor3 = Color3.fromRGB(120, 210, 255)
		end
		-- Highlight tin của bản thân
		if entry.userId == localPlayer.UserId then
			label.TextColor3 = Color3.fromRGB(180, 255, 160)
		end

		totalHeight = totalHeight + lineH + 4
	end
...
local function applyChatMode(mode)
	...
	elseif mode == 3 then
		...
		task.spawn(function()
			...
			local seen = {}
			local toRender = {}
			for i = #globalChatLog, 1, -1 do
				local entry = globalChatLog[i]
				if entry.channelId == ActiveChannelId and not seen[entry.userId] then
...
renderBubble = function(userId, displayName, fullText, channelId)
	...
	if channelId ~= ActiveChannelId then
		-- Vẫn log vào history dù kênh khác
	...
	if existing and existing.frame and existing.frame.Parent then
		...
		-- Tăng unread nếu không phải bản thân và HistoryChat không đang mở cho người này
