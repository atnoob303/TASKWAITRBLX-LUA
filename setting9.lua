	...
	if isAdvance and advanceFrame then
		...
		if KeyBroad then
			...
			if _sharedBroad then
				keyData                   = _sharedBroad.keyData
				closeKeyBroadFn           = _sharedBroad.closeKeyBroad
				cancelKeyTweensFn         = _sharedBroad.cancelKeyTweens
				resetAllKeysFn            = _sharedBroad.resetAllKeys
				openKeyBroadFn            = _sharedBroad.openKeyBroad
				closeKeyBroadDirectFn     = _sharedBroad.closeKeyBroadDirect
				closeKeyBroadFromCenterFn = _sharedBroad.closeKeyBroadFromCenter
				isBroadOpenRef            = _sharedBroad.isBroadOpenRef
				...
				local function openKeyBroad()
					cancelBroadThreads()
					cancelKeyTweens()
					isBroadOpenRef.value = true
					print("[SliderModule.Key] KeyBroad được mở từ setting: " .. title)
				...
				local function closeKeyBroad(onAllDone)
					cancelBroadThreads()
					isBroadOpenRef.value = false
				...
				local function closeKeyBroadFromCenter(centerKeyCode, onAllDone)
					cancelBroadThreads()
					isBroadOpenRef.value = false
				...
				for _, kd in ipairs(keyData) do
					...
					if clickKey then
						table.insert(self._conns, clickKey.MouseEnter:Connect(function()
							makeButtonShine(frameDecor, false, ctxFrame)
						end))
						table.insert(self._conns, clickKey.InputBegan:Connect(function(input)
							if input.UserInputType == Enum.UserInputType.Touch then
								makeButtonShine(frameDecor, false, ctxFrame)
							end
						end))
						table.insert(self._conns, clickKey.MouseButton1Click:Connect(function()
							if not self._waiting then return end
							if not openRegistry[self] then return end
							...
							-- Ẩn lan từ phím cũ ra → hiện lại ngẫu nhiên
			...
			-- ── TableButton: toggle KeyBroad ────────────────────────
			if TableButton then
				local TableDecor = TableButton:FindFirstChild("Decor")
				local ctxTable   = {}

				table.insert(self._conns, TableButton.MouseEnter:Connect(function()
					makeButtonShine(TableDecor, false, ctxTable)
				end))
				table.insert(self._conns, TableButton.InputBegan:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.Touch then
						makeButtonShine(TableDecor, false, ctxTable)
					end
				end))

				table.insert(self._conns, TableButton.MouseButton1Click:Connect(function()
					if not self._waiting then return end
					if not openRegistry[self] then return end
					makeButtonShine(TableDecor, true, ctxTable)
					if isBroadOpenRef.value then
						-- Đóng: A→Z, hiện lại TIPText sau khi xong
		...
		if KeyButton then
			...
			-- Hover → shine (không flash)
			table.insert(self._conns, KeyButton.MouseEnter:Connect(function()
				makeButtonShine(KeyDecor, false, ctxKey)
			end))
			table.insert(self._conns, KeyButton.InputBegan:Connect(function(input)
				if input.UserInputType == Enum.UserInputType.Touch then
					makeButtonShine(KeyDecor, false, ctxKey)
				end
			end))

			table.insert(self._conns, KeyButton.MouseButton1Click:Connect(function()
				if not self._waiting then return end
				if not openRegistry[self] then return end
		...
		if CancelButton then
			-- Decor của CancelButton
			local CancelDecor = CancelButton:FindFirstChild("Decor")
			local ctxCancel   = {}

			-- Hover → shine
			table.insert(self._conns, CancelButton.MouseEnter:Connect(function()
				makeButtonShine(CancelDecor, false, ctxCancel)
			end))
			table.insert(self._conns, CancelButton.InputBegan:Connect(function(input)
				if input.UserInputType == Enum.UserInputType.Touch then
					makeButtonShine(CancelDecor, false, ctxCancel)
				end
			end))

			table.insert(self._conns, CancelButton.MouseButton1Click:Connect(function()
				if not self._waiting then return end
				if not openRegistry[self] then return end
				closeAdvFrame(false)
			end))
