...
-- ── Adv Frame ownership ──────────────────────────────────
local advOwner = nil   -- inst đang giữ advanceFrame
local advConns = {}    -- connections hiện tại trên advanceFrame

local function unmountAdv(inst)
	if advOwner ~= inst then return end
	for _, c in ipairs(advConns) do c:Disconnect() end
	advConns = {}
	advOwner = nil
end

local function mountAdv(inst, setupFn)
	for _, c in ipairs(advConns) do c:Disconnect() end
	advConns = {}
	advOwner = inst
...
local function advConn(conn)
	table.insert(advConns, conn)
end

-- ── KeyBroad ownership ───────────────────────────────────
-- Khi inst A đang giữ KeyBroad, inst B cần dọn sạch trước khi mount
local keyBroadOwner = nil  -- { cancelTweens, resetKeys, closeBroad, broadPos, keyBroad }

local function keyBroad_release()
	if not keyBroadOwner then return end
	local o = keyBroadOwner
	keyBroadOwner = nil
	if o.cancelTweens then o.cancelTweens() end
	if o.closeBroad   then o.closeBroad()   end
	if o.resetKeys    then o.resetKeys()    end
	if o.keyBroad then
		o.keyBroad.Visible = false
		if o.broadPos then
			o.keyBroad.Position = UDim2.new(
				o.broadPos.X.Scale, o.broadPos.X.Offset,
				3, o.broadPos.Y.Offset
			)
		end
	end
end
...
function SliderModule.Key(config)
	...
	-- Quản lý tween đang chạy
	local advTweens = {}
	local function cancelAdvTweens()
		for _, t in ipairs(advTweens) do t:Cancel() end
		advTweens = {}
	end
	local function trackTween(t)
		table.insert(advTweens, t)
		t:Play()
	end
	...
	local function closeAdvFrame()
		...
		-- Đóng bàn phím — clear ownership rồi reset
		if keyBroadOwner and keyBroadOwner.keyBroad == KeyBroad then
			keyBroad_release()
		else
			-- Fallback nếu inst này tự giữ local refs
			if closeKeyBroadFn then closeKeyBroadFn() end
			if KeyBroad then
	...
	local _setupAdvConnsFn = nil  -- set sau khi isAdvance block chạy

	local function openAdvFrame()
		mountAdv(self, _setupAdvConnsFn)
		registerOpen(self)
		self._waiting = true
		...
		-- Nếu inst khác đang giữ KeyBroad → dọn sạch tweens của họ trước
		keyBroad_release()
		if KeyBroad then
			if cancelKeyTweensFn then cancelKeyTweensFn() end
			KeyBroad.Visible = false
			...
			if resetAllKeysFn then resetAllKeysFn() end
			-- Đăng ký ownership của inst này
			keyBroadOwner = {
				cancelTweens = cancelKeyTweensFn,
				closeBroad   = closeKeyBroadFn,
				resetKeys    = resetAllKeysFn,
				keyBroad     = KeyBroad,
				broadPos     = BASE.broadPos,
			}
		end
	...
	if isAdvance and advanceFrame then
		...
		-- Khai báo ngoài if KeyBroad để setupAdvConns luôn thấy table (dù rỗng)
		local keyData = {}

		-- Stub fallback cho các biến KeyBroad — dùng khi KeyBroad = nil
		-- để setupAdvConns không bị lỗi "upvalue not found"
		local isBroadOpen          = false
		local openKeyBroad         = function() end
		local closeKeyBroad        = function(cb) if cb then cb() end end
		local closeKeyBroadFromCenter = function(_, cb) if cb then cb() end end
		...
		if KeyBroad then
			...
			local broadPosTween = nil  -- track riêng tween position KeyBroad
			openKeyBroad = function()
				if broadPosTween then broadPosTween:Cancel(); broadPosTween = nil end
				cancelBroadThreads()
				cancelKeyTweens()
				-- Reset tất cả phím về ẩn hoàn toàn trước khi show lại
				...
				if BASE.broadPos then
					broadPosTween = TweenService:Create(KeyBroad, BROAD_OPEN, { Position = BASE.broadPos })
					trackTween(broadPosTween)
				end
				...
				-- Reset tất cả phím về transparent trước khi show lại
				-- (quan trọng cho lần 2+ vì phím còn visible từ lần trước)
				cancelKeyTweens()
				for _, kd in ipairs(keyData) do
					kd.frame.BackgroundTransparency = 1
					kd.keyLabel.TextTransparency    = 1
				...
				for i, kd in ipairs(shuffled) do
					local thread = task.delay((i - 1) * STEP, function()
						if not isBroadOpen then return end
						showKey(kd)
					end)
			...
			closeKeyBroad = function(onAllDone)
				cancelBroadThreads()
				isBroadOpen = false
				...
				if BASE.broadPos then
					...
					broadPosTween = TweenService:Create(KeyBroad, BROAD_CLOSE, { Position = offPos })
					trackTween(broadPosTween)
				end
			...
			closeKeyBroadFromCenter = function(centerKeyCode, onAllDone)
				cancelBroadThreads()
				isBroadOpen = false
		...
		-- ── setupAdvConns: reconnect tất cả adv button events ───
		-- Gọi mỗi lần instance này mount vào advanceFrame
		local function setupAdvConns()
			if isAdvance and KeyBroad then
				for _, kd in ipairs(keyData) do
					...
					if clickKey then
						advConn(clickKey.MouseEnter:Connect(function()
							makeButtonShine(frameDecor, false, ctxFrame)
						end))
						advConn(clickKey.InputBegan:Connect(function(input)
							if input.UserInputType == Enum.UserInputType.Touch then
								makeButtonShine(frameDecor, false, ctxFrame)
							end
						end))
						advConn(clickKey.MouseButton1Click:Connect(function()
							if not self._waiting then return end
							if blacklist[keyCode] then return end
							local prev = self._preview
							self._preview = keyCode
							if AdvKeyText then
								AdvKeyText.Text       = getKeyName(keyCode)
								AdvKeyText.TextColor3 = getKeyColor(keyCode)
							end
							local fromCenter = prev or keyCode
							closeKeyBroadFromCenter(fromCenter, function()
								if self._waiting then openKeyBroad() end
							end)
						end))
					end
				end
			...
			if TableButton then
				...
				local ctxTable   = {}
				advConn(TableButton.MouseEnter:Connect(function()
					makeButtonShine(TableDecor, false, ctxTable)
				end))
				advConn(TableButton.InputBegan:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.Touch then
						makeButtonShine(TableDecor, false, ctxTable)
					end
				end))
				advConn(TableButton.MouseButton1Click:Connect(function()
					makeButtonShine(TableDecor, true, ctxTable)
					if isBroadOpen then
						closeKeyBroad(function()
			...
			if KeyButton then
				...
				local ctxKey   = {}
				advConn(KeyButton.MouseEnter:Connect(function()
					makeButtonShine(KeyDecor, false, ctxKey)
				end))
				advConn(KeyButton.InputBegan:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.Touch then
						makeButtonShine(KeyDecor, false, ctxKey)
					end
				end))
				advConn(KeyButton.MouseButton1Click:Connect(function()
					if not self._preview then return end
					local confirmed = self._preview
					closeAdvFrame()
			...
			if CancelButton then
				...
				local ctxCancel   = {}
				advConn(CancelButton.MouseEnter:Connect(function()
					makeButtonShine(CancelDecor, false, ctxCancel)
				end))
				advConn(CancelButton.InputBegan:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.Touch then
						makeButtonShine(CancelDecor, false, ctxCancel)
					end
				end))
				advConn(CancelButton.MouseButton1Click:Connect(function()
					closeAdvFrame()
				end))
			end
		end
		_setupAdvConnsFn = setupAdvConns  -- gán cho openAdvFrame dùng
	end
