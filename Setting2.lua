...
function SliderModule.Key(config)
	...
	-- ── Decor shiny effect ───────────────────────────────────
	local DecorGradient    = Decor and Decor:FindFirstChildWhichIsA("UIGradient")
	local decorTween1, decorTween2
	local colorTween1, colorTween2

	local BASE_OFFSET   = Vector2.new(-0.3, 0)
	local BASE_BG_COLOR = Decor and Decor.BackgroundColor3
	local BASE_BG_TRANS = Decor and Decor.BackgroundTransparency

	-- withFlash: true = kèm hiệu ứng sáng trắng (khi đổi key thành công)
	local function playDecorShine(withFlash)
		if not DecorGradient then return end
		...
		-- Reset về vị trí cơ bản trước khi bắt đầu
		DecorGradient.Offset = BASE_OFFSET
		...
		-- Phase 1: đi xuống BASE_OFFSET → (1, 1)
		decorTween1 = TweenService:Create(DecorGradient, infoDown, {
			Offset = Vector2.new(0.7, 0.7),
		})

		-- Phase 1 kèm flash: bg đen → trắng, trans 0.8 → 0
		if withFlash and Decor then
			if colorTween2 then colorTween2:Cancel() end
			Decor.BackgroundColor3       = Color3.fromRGB(0, 0, 0)
			Decor.BackgroundTransparency = 0.8
			colorTween1 = TweenService:Create(Decor, infoDown, {
		...
		decorTween1.Completed:Connect(function(state)
			if state ~= Enum.PlaybackState.Completed then return end

			-- Snap ngay lên (-1, -1) không tween
			DecorGradient.Offset = Vector2.new(-1, -1)

			-- Reset màu ngay lập tức nếu có flash
			if withFlash and Decor then
				if colorTween1 then colorTween1:Cancel() end
				Decor.BackgroundTransparency = 0.8
				colorTween2 = TweenService:Create(Decor, infoDown, {
					BackgroundColor3       = Color3.new(0, 0, 0),
					BackgroundTransparency = BASE_BG_TRANS,
				})
				colorTween2:Play()
			end

			-- Phase 2: về vị trí cơ bản, không tween màu
			decorTween2 = TweenService:Create(DecorGradient, infoUp, {
				Offset = BASE_OFFSET,
			})
			decorTween2:Play()
		end)
	...
	local self = setmetatable({
		...
		_title     = title,
		_tag       = config.tag,
		_value     = defaultKey,           -- Enum.KeyCode
		_onChange  = config.onChange,
		_conns     = {},
		_waiting   = false,                -- đang chờ phím không
	}, SliderModule)

	-- ── Helpers ──────────────────────────────────────────────
	local function getKeyName(keyCode)
		-- Lấy tên ngắn gọn: "Enum.KeyCode.LeftShift" → "LeftShift"
	...
	local function applyKey(keyCode, fireEvent)
		...
		-- Shine + flash trắng khi đổi key thành công
		if fireEvent then
			playDecorShine(true)
		end
	...
	local function enterWaiting()
		...
		-- Visual feedback: làm mờ ChangeButton để báo hiệu đang chờ
		TweenService:Create(ChangeButton,
			TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
	...
	table.insert(self._conns, UserInputService.InputBegan:Connect(function(input, gameProcessed)
		...
		-- Bỏ qua click chuột, chỉ nhận bàn phím
		if input.UserInputType ~= Enum.UserInputType.Keyboard then return end
		-- Bỏ qua các phím modifier đơn thuần
		local ignored = {
	...
	-- ── Public API ───────────────────────────────────────────
	function self:getValue()
		return self._value   -- Enum.KeyCode
	end
...
function SliderModule.New(config)
	...
	assert(t == "Slider" or t == "SliderButton" or t == "Enabled"
		...
		'[SliderModule.New] type phải là "Slider", "SliderButton", "Enabled", "Selected", "Color", hoặc "Gradient", nhận: ' .. tostring(t))
