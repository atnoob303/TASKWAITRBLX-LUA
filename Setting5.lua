...
function SliderModule.Key(config)
	...
	if isAdvance and advanceFrame then
		...
		if KeyBroad then
			...
			for _, keyFrame in ipairs(KeyBroad:GetChildren()) do
				if not keyFrame:IsA("Frame") then continue end

				local keyLabel = keyFrame:FindFirstChild("Key")
				if not keyLabel then continue end

				-- Đọc text rồi lookup trong KEY_MAP
				local labelText = keyLabel.Text:upper():gsub("%s+", "")
				local entry     = KEY_MAP[labelText]
				if not entry then continue end

				local keyCode = entry.keyCode

				-- Đặt tên frame + LayoutOrder
				keyFrame.Name        = "Key_" .. labelText:lower()
